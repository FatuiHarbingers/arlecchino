...
import { bold, hyperlink, strikethrough, time, TimestampStyles } from '@discordjs/formatters'
import type { TFunction } from 'i18next'
import { isIPv4 } from 'net'
import { ActivityFormatter } from './ActivityFormatter'

export class LogEventsFormatter extends ActivityFormatter<LogEventsItem> {
	public createEmbed( item: LogEventsItem, t: TFunction ): EmbedBuilder {
		...
		if ( item.isBlock() ) {
			...
			const description = t( `general:${ i18nKey }`, {
				replace: {
					author,
					target
				}
			...
			if ( item.isBlocking() || item.isReblocking() ) {
				...
				const duration = !expiry || isNaN( expiry as unknown as number )
					? t( 'general:log-block-expiry-infinite' )
					: time( expiry, TimestampStyles.RelativeTime )
				embed.addFields( {
					name: t( 'general:log-block-expiry' ),
					value: duration
				} )
			}
			...
			const description = t( `general:log-delete-${ i18nKey }`, {
				replace: {
					author,
					target
				}
			...
			const description = t( `general:log-move-${ i18nKey }`, {
				replace: {
					author,
					from,
					to
			...
			const description = t( `general:log-protect-${ i18nKey }`, {
				replace: {
					author,
					target
				}
			...
			if ( item.isProtecting() || item.isModifying() ) {
				embed.addFields( {
					name: t( 'general:log-protect-details' ),
			...
			const description = t( 'general:log-thanks', {
				author,
				target
			} )
			embed.setDescription( description )
			...
			const description = t( 'general:log-rights', {
				replace: {
					author,
					target
				}
			...
			if ( params.oldmetadata.length === 0 ) {
				oldData = t( 'general:log-rights-none' )
			} else {
				oldData = params.oldmetadata.map( item => {
					...
					if ( item.expiry !== 'infinity' ) {
						...
						detail += ` ${  t( 'general:log-rights-until', {
							replace: {
			...
			if ( params.newmetadata.length === 0 ) {
				newData = t( 'general:log-rights-none' )
			} else {
			...
			embed.addFields(
				{
					inline: true,
					name: t( 'general:log-rights-old-groups' ),
					value: oldData
				},
				{
					inline: true,
					name: t( 'general:log-rights-new-groups' ),
					value: newData
				}
			)
		} else {
			...
			const description = t( `general:log-upload-${ i18nKey }`, {
				replace: {
					author,
					target
				}
